...
The `home-routes` handler in the `bass4.routes.home` namespace
...
The `home-page` function will in turn call the `bass4.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `bass4.handler` namespace:
...
Request middleware functions are located under the `bass4.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `bass4.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `bass4.db.core` in some other namespace.
