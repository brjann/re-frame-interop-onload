(ns bass4.test.bankid.mock-backend
...
(defn create-session!
  ...
  (let [order-ref (UUID/randomUUID)]
    (swap! mock-backend-sessions add-session-map personnummer order-ref)
    order-ref))

(defn update-session!
  [order-ref session]
  (swap!
    mock-backend-sessions
    (fn [all-sessions]
...
(defn session-by-personnummer
  ...
  (if-let [order-ref (get-in @mock-backend-sessions [:by-personnummer personnummer])]
    (get-in @mock-backend-sessions [:sessions order-ref])
...
(defn api-auth
  ...
  ([personnummer user-ip _]
   ...
   (let [existing-order-ref (get-in @mock-backend-sessions [:by-personnummer personnummer])
         existing-session   (get-in @mock-backend-sessions [:sessions existing-order-ref])]
