...
(deftest group
  (let [group-id (create-group!)
        ...
        user2-id (user-service/create-user! project-id {:group group-id})]
    ; Today
    (create-group-administration!
      group-id group-single 1 {:date (midnight *now*)})
    (create-group-administration!
      group-id ass-group-weekly 4 {:date (midnight *now*)})
    ; Wrong scope
    (create-group-administration!
      group-id ass-individual-single 1 {:date (midnight *now*)})
    ; Tomorrow
    (create-group-administration!
      group-id ass-group-weekly 1 {:date (+ (midnight+d 1 *now*))})
