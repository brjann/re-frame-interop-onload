DROP PROCEDURE IF EXISTS create_bass_objects_without_parent;
DELIMITER //
CREATE PROCEDURE create_bass_objects_without_parent
(IN NewClass CHAR(45), ParentProperty CHAR(100), HowMany INT)
BEGIN

	DECLARE TableName CHAR(100);
    DECLARE ParentId, x, NewObjectId, NewSortOrderMax, NewParentInterface INT;
   ...
   #     SET NewParentInterface = @ParentInterface;
		...
		SET objectlist_sql = "INSERT INTO objectlist (objectid, classname) VALUES";
		SET class_sql = concat("INSERT INTO ", TableName, " (ObjectId, ClassName, Changed, Created, ParentProperty, ParentInterface, SortOrder)", " VALUES");
		WHILE x <= HowMany DO
			SET NewSortOrderMax = NewSortOrderMax + 5;
			SET NewObjectId = NewObjectId + 1;

			SET objectlist_sql = concat(objectlist_sql, concat("(", NewObjectId, ",'", NewClass, "')"));
			SET class_sql = concat(class_sql, concat("(", NewObjectId, ",'", NewClass, "',", unix_timestamp(now()), ",", unix_timestamp(now()), ",'", ParentProperty, "',", NewParentInterface, ",", NewSortOrderMax, ")"));
			IF x < HowMany THEN
